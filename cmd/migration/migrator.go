...
func (m *Migrator) Up() error {
	...
	for _, model := range m.models {
		...
		m.logger.Info("Migrating model", zap.String("model", modelName))

		if err := m.db.AutoMigrate(model); err != nil {
			m.logger.Error("Failed to migrate model", zap.String("model", modelName), zap.Error(err))
			return err
		}
...
func (m *Migrator) Down() error {
	...
	for i := len(m.models) - 1; i >= 0; i-- {
		...
		m.logger.Info("Dropping table for model", zap.String("model", modelName))

		if err := m.db.Migrator().DropTable(model); err != nil {
			m.logger.Error("Failed to drop table", zap.String("model", modelName), zap.Error(err))
			return err
		}
